...
//----------------------------------
    ...
    self.readUnsignedByte = function () {
        _position++;
        return _dv.getUint8(_position - 1, BIG_ENDIAN);
    }
    ...
    self.readUnsignedInt = function () {
        _position += 4;
        return _dv.getUint32(_position - 4, BIG_ENDIAN);
    }
    ...
    self.readUTFBytes = function (len) {
        var bytes = self.readBytes(len)
        return UTF8.getStringFromBytes(bytes);
    }
    self.readBin = function (len) {
        var bytes = self.readBytes(len)
        return bytes;
    }
    self.readUnsignedShort = function () {
        _position+= 2;
        return _dv.getUint16(_position - 2, BIG_ENDIAN);
    }
    self.readShort = function () {
        _position+= 2;
        return _dv.getInt16(_position - 2, BIG_ENDIAN);
    }
    /**
     * 获取ByteArray里的字节数据
     */
    ...
    self.bytesAvailable = function () {
        return _bytesLength-_position;
    }
};
